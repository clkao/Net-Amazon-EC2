...
is($key_pair->{keyName}, "test_keys", "Does new key pair come back?");

# describe_key_pairs
my $key_pairs       = $ec2->describe_key_pairs;
my $seen_test_key   = 0;
foreach my $key_pair (@{$key_pairs}) {
     if ($key_pair->{keyName} eq "test_keys") {
         $seen_test_key = 1;
     }
}
...
foreach my $security_group (@{$security_groups}) {
    if ($security_group->{groupName} eq "test_group") {
        $seen_test_group = 1;
        if ($security_group->{ipPermissions}{item}[0]{ipRanges}{item}[0]{cidrIp} eq '10.253.253.253/32') {
            $seen_test_rule = 1;
        }
    }
}
...
ok($run_result->{groups}[0] eq "test_group", "Checking for running instance");

my $instance_id = $run_result->{instance}[0]{instanceId};
 
# describe_instances
my $running_instances = $ec2->describe_instances();
my $seen_test_instance = 0;
foreach my $instance (@{$running_instances}) {
    my $instance_set = $instance->{instance}[0];
    if ($instance_set->{keyName} eq "test_keys" and $instance_set->{imageId} eq "ami-26b6534f") {
...
is($terminate_result->[0]{instanceId}, $instance_id, "Checking to see if instance was terminated successfully");
